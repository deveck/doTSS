    ...
    Available commands:
        ...
        public override void Execute (string[] commandline)
        {
			...
			if (keyCommand == "keystore_open")
			{
    			...
    			if(_console.GetValue<TPMKeystoreProvider>(localAlias + "_keystore", null) != null)
    			{
    			...
    			{
    				for(int i = 1; i<sArguments.Length; i++)
    				{
    					string[] splittedArgument = sArguments[i].Split('=');
    			...
    			_console.SetValue(localAlias + "_keystore", keystore);   			
    			
    			
        	}
        	else if (keyCommand == "keystore_close")
			{
				TPMKeystoreProvider keystore = _console.GetValue<TPMKeystoreProvider>(localAlias + "_keystore", null);
				
				if(keystore != null)
				{
					keystore.Dispose();
					_console.ClearValue(localAlias + "_keystore");
				}
